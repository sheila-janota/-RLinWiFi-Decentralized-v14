...
class Agent:
    ...
    def __init__(self, state_size, action_size, stationNum=5, config=Config(), random_seed=42, actor_layers=None, critic_layers=None):
        ...
        if actor_layers is None:
            self.actor_local = Actor(
                state_size, action_size, "actor_local"+str(stationNum), random_seed, config.BATCH_SIZE).to(device)
            print("actor_local"+str(stationNum))
            self.actor_target = Actor(
                state_size, action_size, "actor_target"+str(stationNum), random_seed, config.BATCH_SIZE).to(device)
            print("actor_target"+str(stationNum))
        else:
            self.actor_local = Actor(
                state_size, action_size, "actor_local"+str(stationNum), random_seed, config.BATCH_SIZE, *actor_layers).to(device)
            print("actor_local"+str(stationNum))
            self.actor_target = Actor(
                state_size, action_size, "actor_target"+str(stationNum), random_seed, config.BATCH_SIZE, *actor_layers).to(device)
            print("actor_target"+str(stationNum))

        if critic_layers is None:
            self.critic_local = Critic(
                state_size, action_size, "critic_local"+str(stationNum), random_seed, config.BATCH_SIZE).to(device)
            print("critic_local"+str(stationNum))
            self.critic_target = Critic(
                state_size, action_size, "critic_target"+str(stationNum), random_seed, config.BATCH_SIZE).to(device)
            print("critic_target"+str(stationNum))
        else:
            self.critic_local = Critic(
                state_size, action_size, "critic_local"+str(stationNum), random_seed, config.BATCH_SIZE, *critic_layers).to(device)
            print("critic_local"+str(stationNum))
            self.critic_target = Critic(
                state_size, action_size, "critic_target"+str(stationNum), random_seed, config.BATCH_SIZE, *critic_layers).to(device)
            print("critic_target"+str(stationNum))
