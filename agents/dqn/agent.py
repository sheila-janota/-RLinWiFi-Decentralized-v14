...
class Agent:
    ...
    def __init__(self, network, state_size, action_size, stationNum=5, config=Config(), seed=42, save=True, save_loc='models/', save_every=2, checkpoint_file=None):
        ...
        self.qnetwork_local = network(
            self.sess, state_size, action_size, "local"+str(stationNum), self.config.LR, checkpoint_file)
        print(self.qnetwork_local)
        print("local"+str(stationNum))
        self.qnetwork_target = network(
            self.sess, state_size, action_size, "target"+str(stationNum), self.config.LR, checkpoint_file)
        print(self.qnetwork_target)
        print("target"+str(stationNum))
