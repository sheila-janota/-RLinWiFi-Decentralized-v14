...
class Logger:
    ...
    def log_episode(self, cumulative_reward, speed, step):
        if self.send_logs:
            print("Train Method-cumulative_reward: ", cumulative_reward)
        ...
        for i in range(EPISODE_COUNT):
            ...
            with tqdm.trange(steps_per_ep) as t:
                ...
                for step in t:
                    ...
                    #    csv_writer.writerow(['Execution Time'])  # Write a header
                    ...
                    #print("mean_execution_time: ", mean_execution_time)
            ...
            print("execution_time media:", np.mean(execution_times))
            self.env.close()
            if experimental:
                self.env = EnvWrapper(self.env.no_threads, **self.env.params)
...
class EnvWrapper:
    ...
    def _craft_commands(self, params):
        ...
        command = f'{waf_pwd} --run "RLinWiFi-Decentralized-v14-30-station'
